...
app = Flask(__name__)
app.secret_key = os.getenv('SECRET_KEY')  # Set secret key for session
...
# Route to handle user's choice
@app.route('/choose', methods=['POST'])
def choose():
    choice = request.form.get('choice')
    if choice == 'new':
        return redirect(url_for('login', game='new'))
    elif choice == 'continue':
        return redirect(url_for('login', game='continue'))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if game == 'new':
                ...
                return redirect(url_for('index'))  # Adjust this if you have a different route for continuing a game
...
def index():
    ...
    return render_template('index.html', bucket_url=bucket_url)
